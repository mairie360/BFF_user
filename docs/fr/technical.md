...
| `TRUST_PROXY` | non défini (aucun proxy de confiance) | `trust proxy` d’Express: nombre de sauts (`1`), `true`, ou adresses/sous-réseaux de confiance (`loopback, 10.0.0.0/8`). À définir derrière l’ingress pour que les limites s’appliquent par client et non par proxy. |
...
| `AUTH_RATE_LIMIT_MAX` | `10` | Connexions échouées par IP cliente et e-mail de compte par fenêtre. |
| `AUTH_RATE_LIMIT_IP_MAX` | `100` | Tentatives échouées par IP cliente par fenêtre, sur `/auth/login` et `/auth/force_change_password`. |
