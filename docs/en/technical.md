...
| `TRUST_PROXY` | unset (no proxy trusted) | Express `trust proxy`: hop count (`1`), `true`, or trusted addresses/subnets (`loopback, 10.0.0.0/8`). Set it behind the ingress so rate limits apply per client and not per proxy. |
...
| `AUTH_RATE_LIMIT_MAX` | `10` | Failed logins per client IP and account e-mail per window. |
| `AUTH_RATE_LIMIT_IP_MAX` | `100` | Failed attempts per client IP per window, over `/auth/login` and `/auth/force_change_password`. |
